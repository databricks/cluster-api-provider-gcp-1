...
func setupReconcilers(ctx context.Context, mgr ctrl.Manager) error {
	...
	if feature.Gates.Enabled(feature.GKE) {
		...
		if err := (&expcontrollers.GCPManagedClusterReconciler{
			...
			ReconcileTimeout:        reconcileTimeout,
			WatchFilterValue:        watchFilterValue,
			GcpmcReconcileWhitelist: gcpmcReconcileWhitelist,
		...
		if err := (&expcontrollers.GCPManagedControlPlaneReconciler{
			...
			ReconcileTimeout:         reconcileTimeout,
			WatchFilterValue:         watchFilterValue,
			GcpmcpReconcileWhitelist: gcpmcpReconcileWhitelist,
		}).SetupWithManager(ctx, mgr, controller.Options{
			MaxConcurrentReconciles: gcpClusterConcurrency,
		...
		if err := (&expcontrollers.GCPManagedMachinePoolReconciler{
			...
			ReconcileTimeout:         reconcileTimeout,
			WatchFilterValue:         watchFilterValue,
			GcpmmpReconcileWhitelist: gcpmmpReconcileWhitelist,
		}).SetupWithManager(ctx, mgr, controller.Options{
...
func initFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&gcpmcReconcileWhitelist,
		"gcpmc-reconcile-whitelist",
		"",
		"Comma-separated list of GCPMC CRs to reconcile. If not set, the operator will reconcile all GCPMC CRs.",
	)

	fs.StringVar(&gcpmcpReconcileWhitelist,
		"gcpmcp-reconcile-whitelist",
		"",
		"Comma-separated list of GCPMCP CRs to reconcile. If not set, the operator will reconcile all GCPMCP CRs.",
	)

	fs.StringVar(&gcpmmpReconcileWhitelist,
		"gcpmmp-reconcile-whitelist",
		"",
		"Comma-separated list of GCPMMP CRs to reconcile. If not set, the operator will reconcile all GCPMMP CRs.",
	)
