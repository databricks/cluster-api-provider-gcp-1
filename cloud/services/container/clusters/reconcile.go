...
func (s *Service) createCluster(ctx context.Context, log *logr.Logger) error {
	...
	if !s.scope.IsAutopilotCluster() {
		cluster.NodePools = scope.ConvertToSdkNodePools(s.scope.GCPManagedControlPlane.Spec.Location, nodePools, machinePools, cluster.Name)
	}
	...
	if s.scope.GCPManagedControlPlane.Spec.AutoscalingProfile != nil {
		cluster.Autoscaling = &containerpb.ClusterAutoscaling{
			AutoscalingProfile: infrav1exp.ConvertToSdkAutoscalingProfile(s.scope.GCPManagedControlPlane.Spec.AutoscalingProfile),
		}
	}
...
func (s *Service) checkDiffAndUpdateCluster(ctx context.Context, existingCluster *containerpb.Cluster, log *logr.Logger) (bool, error) {
	...
	needUpdateAutoscalingProfile, updateAutoscalingProfileRequest := s.checkDiffAndPrepareUpdateAutoscalingProfile(existingCluster, log)
	if needUpdateAutoscalingProfile {
		log.Info("AutoscalingProfile update required")
		err := s.updateCluster(ctx, updateAutoscalingProfileRequest, log)
		if err != nil {
			return false, err
		}
		log.Info("cluster AutoscalingProfile updating in progress")
		return true, nil
	}
...
// ensures a version string contains a "v" prefix to be a valid semver string
func semverStr(v string) string {
...
func (s *Service) checkDiffAndPrepareUpdateMasterVersion(existingCluster *containerpb.Cluster, log *logr.Logger) (bool, *containerpb.UpdateClusterRequest) {
	...
	if s.scope.GCPManagedControlPlane.Spec.ControlPlaneVersion != nil {
		...
		currentMasterVersion := infrav1exp.ConvertFromSdkVersion(existingCluster.CurrentMasterVersion)
		// Only upgrade if the desired version is greater than the current version
		// Desired version could be lower than the current version, this happens after GKE automatically upgrades the cluster to a newer patch version
		// In that case, we don't want to downgrade because it would fail and prevent further reconciliation (such as updating maintenance exclusions).
		if desiredMasterVersion != currentMasterVersion && semver.Compare(semverStr(desiredMasterVersion), semverStr(currentMasterVersion)) > 0 {
			needUpdate = true
...
func (s *Service) checkDiffAndPrepareUpdateAutoscalingProfile(existingCluster *containerpb.Cluster, log *logr.Logger) (bool, *containerpb.UpdateClusterRequest) {
	needUpdate := false
	clusterUpdate := containerpb.ClusterUpdate{}

	desiredAutoscalingProfile := infrav1exp.ConvertToSdkAutoscalingProfile(s.scope.GCPManagedControlPlane.Spec.AutoscalingProfile)
	if desiredAutoscalingProfile != containerpb.ClusterAutoscaling_PROFILE_UNSPECIFIED {
		if existingCluster.Autoscaling != nil {
			if desiredAutoscalingProfile != existingCluster.Autoscaling.AutoscalingProfile {
				needUpdate = true
				clonedAutoscaling := proto.Clone(existingCluster.Autoscaling).(*containerpb.ClusterAutoscaling)
				clonedAutoscaling.AutoscalingProfile = desiredAutoscalingProfile
				clusterUpdate.DesiredClusterAutoscaling = clonedAutoscaling
				log.V(2).Info("AutoscalingProfile update required", "current", existingCluster.Autoscaling, "desired", desiredAutoscalingProfile)
			}
		} else if desiredAutoscalingProfile != containerpb.ClusterAutoscaling_BALANCED {
			needUpdate = true
			clusterUpdate.DesiredClusterAutoscaling = &containerpb.ClusterAutoscaling{
				AutoscalingProfile: desiredAutoscalingProfile,
			}
			log.V(2).Info("AutoscalingProfile update required", "current", nil, "desired", desiredAutoscalingProfile)
		}
	}
	...
	log.V(4).Info("AutoscalingProfile update cluster request. ", "needUpdate", needUpdate, "updateClusterRequest", &updateClusterRequest)
	return needUpdate, &updateClusterRequest
}
