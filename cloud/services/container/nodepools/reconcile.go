...
import (
	...
	"sigs.k8s.io/cluster-api-provider-gcp/util/nodepool"
...
func (s *Service) createNodePool(ctx context.Context, log *logr.Logger) error {
	...
	createNodePoolRequest := &containerpb.CreateNodePoolRequest{
		NodePool: scope.ConvertToSdkNodePool(*s.scope.GCPManagedMachinePool, *s.scope.MachinePool, s.scope.NumZones(), s.scope.GCPManagedControlPlane.Spec.ClusterName),
...
func FilterTaints(taints []*containerpb.NodeTaint, excludeGPU bool) []*containerpb.NodeTaint {
	if !excludeGPU {
		return taints
	}
	var filtered []*containerpb.NodeTaint
...
func (s *Service) checkDiffAndPrepareUpdateConfig(existingNodePool *containerpb.NodePool, log *logr.Logger) (bool, *containerpb.UpdateNodePoolRequest) {
	...
	desiredNodePool := scope.ConvertToSdkNodePool(*s.scope.GCPManagedMachinePool, *s.scope.MachinePool, s.scope.NumZones(), s.scope.GCPManagedControlPlane.Spec.ClusterName)

	// Node version
	if s.scope.NodePoolVersion() != nil && infrav1exp.ConvertFromSdkVersion(*s.scope.NodePoolVersion()) != infrav1exp.ConvertFromSdkVersion(existingNodePool.Version) {
		needUpdate = true
		log.Info("Node pool version differs from desired version", "current", existingNodePool.Version, "desired", infrav1exp.ConvertFromSdkVersion(*s.scope.NodePoolVersion()))
		updateNodePoolRequest.NodeVersion = infrav1exp.ConvertFromSdkVersion(*s.scope.NodePoolVersion())
	}
	// Kubernetes labels
	if !reflect.DeepEqual(desiredNodePool.Config.GetLabels(), existingNodePool.Config.Labels) {
		needUpdate = true
		log.Info("Node pool labels differ from desired labels", "current", existingNodePool.Config.Labels, "desired", desiredNodePool.Config.Labels)
	...
	// Kubernetes taints
	existingTaints := existingNodePool.Config.Taints
	desiredTaints := desiredNodePool.Config.GetTaints()

	// It looks like GKE automatically adds the taint "nvidia.com/gpu=present:NoSchedule" to some GPU node pools.
	// This would trigger infinite updates, as 1) some node pool spec doesn't have this taint,
	// and 2) some node pool spec have a slightly different taint "nvidia.com/gpu=true:NoSchedule"
	// We need to filter out this taint from the diffing logic if it already exists.
	// Services in the universe code base currently only checks for the existence of the taint key "nvidia.com/gpu"
	// so this should not affect existing services.
	hasGpuTaintInExisting := false
	for _, t := range existingTaints {
		if t.Key == "nvidia.com/gpu" {
			hasGpuTaintInExisting = true
			break
		}
	}
	existingTaints = FilterTaints(existingTaints, hasGpuTaintInExisting)
	desiredTaints = FilterTaints(desiredTaints, hasGpuTaintInExisting)
	if !reflect.DeepEqual(desiredTaints, existingTaints) {
		needUpdate = true
		log.Info("Node pool taints differ from desired taints", "current", existingTaints, "desired", desiredTaints)
		updateNodePoolRequest.Taints = &containerpb.NodeTaints{
			Taints: desiredTaints,
		}
	}
	...
	if desiredNodePool.Config.ImageType != "" && !strings.EqualFold(desiredNodePool.Config.ImageType, existingNodePool.Config.ImageType) {
		needUpdate = true
		log.Info("Node pool image type differs from desired image type", "current", existingNodePool.Config.ImageType, "desired", desiredNodePool.Config.ImageType)
		updateNodePoolRequest.ImageType = desiredNodePool.Config.ImageType
	}
	// Network tags
	if !reflect.DeepEqual(desiredNodePool.Config.Tags, existingNodePool.Config.Tags) {
		needUpdate = true
		log.Info("Node pool network tags differ from desired tags", "current", existingNodePool.Config.Tags, "desired", desiredNodePool.Config.Tags)
		updateNodePoolRequest.Tags = &containerpb.NetworkTags{
			Tags: desiredNodePool.Config.Tags,
		}
	}

	// Recently, Google has started automatically adding its own labels to GKE node pools. These labels are prefixed with "goog-".
	// We need to remove them from the "existing" labels we get from GKE before comparing.
	// Otherwise, we will always find a difference between the desired and existing labels and keep trying to update the node pool.
	filteredLabels := make(map[string]string)
	for key, value := range existingNodePool.Config.ResourceLabels {
		if !strings.HasPrefix(key, "goog-") {
			filteredLabels[key] = value
		}
	}

	// Additional resource labels
	if !reflect.DeepEqual(desiredNodePool.Config.ResourceLabels, filteredLabels) {
		needUpdate = true
		log.Info("Node pool resource labels differ from desired resource labels", "current", existingNodePool.Config.ResourceLabels, "desired", desiredNodePool.Config.ResourceLabels)
...
func (s *Service) checkDiffAndPrepareUpdateSize(existingNodePool *containerpb.NodePool) (bool, *containerpb.SetNodePoolSizeRequest) {
	...
	replicas := nodepool.PerLocationReplicaCount(*s.scope.MachinePool.Spec.Replicas, s.scope.NumZones())

	if desiredAutoscaling.Enabled {
		// In GKE, a node pool size can be smaller than the minimum number of nodes specified. https://cloud.google.com/kubernetes-engine/docs/how-to/cluster-autoscaler#node_pool_size_mismatching
		// To ensure that the node pool size is always greater or equal than the minimum number of nodes specified, we need to update the node pool size.
		if s.scope.GCPManagedMachinePool.Status.Replicas < desiredAutoscaling.TotalMinNodeCount {
			needUpdate = true
			setNodePoolSizeRequest.NodeCount = replicas
		}
	} else if replicas != existingNodePool.InitialNodeCount {
		// When autoscaling is disabled, we update the size whenever the desired replicas is different from the current size.
		needUpdate = true
		setNodePoolSizeRequest.NodeCount = replicas
	}
