...
func ConvertToSdkNodePool(nodePool infrav1exp.GCPManagedMachinePool, machinePool clusterv1exp.MachinePool, numZones int, clusterName string) *containerpb.NodePool {
	replicas := nodepool.PerLocationReplicaCount(*machinePool.Spec.Replicas, numZones)
	...
	if nodePool.Spec.Locations != nil {
		sdkNodePool.Locations = nodePool.Spec.Locations
	}
	if nodePool.Spec.Reservation != nil {
		sdkNodePool.Config.ReservationAffinity = &containerpb.ReservationAffinity{
			ConsumeReservationType: containerpb.ReservationAffinity_SPECIFIC_RESERVATION,
			Key:                    "compute.googleapis.com/reservation-name",
			Values:                 []string{*nodePool.Spec.Reservation},
		}
	}
	if nodePool.Spec.Accelerators != nil {
		sdkNodePool.Config.Accelerators = []*containerpb.AcceleratorConfig{}
		for _, acceleratorConfig := range nodePool.Spec.Accelerators {
			sdkNodePool.Config.Accelerators = append(sdkNodePool.Config.Accelerators, &containerpb.AcceleratorConfig{
...
func ConvertToSdkNodePools(clusterLocation string, nodePools []infrav1exp.GCPManagedMachinePool, machinePools []clusterv1exp.MachinePool, clusterName string) []*containerpb.NodePool {
	res := []*containerpb.NodePool{}
	for i := range nodePools {
		res = append(res, ConvertToSdkNodePool(nodePools[i], machinePools[i], nodepool.NumZones(nodePools[i].Spec.Locations, clusterLocation), clusterName))
	}
	return res
}
...
func (s *ManagedMachinePoolScope) NumZones() int {
	return nodepool.NumZones(s.GCPManagedMachinePool.Spec.Locations, s.GCPManagedControlPlane.Spec.Location)
}
