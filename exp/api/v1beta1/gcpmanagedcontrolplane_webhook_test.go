...
var _ = Describe("GCPManagedControlPlaneWebhook", func() {
	...
	When("ValidatingGCPManagedControlPlaneDelete", func() {
		It("should allow deletion if no validation errors", func() {
			controlPlane := &GCPManagedControlPlane{
				ObjectMeta: controlPlane.ObjectMeta,
				Spec:       controlPlane.Spec,
			}
			err := controlPlane.ValidateDelete()
			Expect(err).To(BeNil())
		})
		It("should disallow deletion if PreventAccidentalDeletion annotation is set", func() {
			controlPlane.ObjectMeta.Annotations[infrav1.PreventAccidentalDeletionAnnotation] = "true"
			controlPlane := &GCPManagedControlPlane{
				ObjectMeta: controlPlane.ObjectMeta,
				Spec:       controlPlane.Spec,
			}
			...
			Expect(err.Error()).To(ContainSubstring(infrav1.PreventAccidentalDeletionAnnotation))
		})
	})
})
