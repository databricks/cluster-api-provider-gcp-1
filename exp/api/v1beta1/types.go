...
// ConvertToSdkAutoscalingProfile converts autoscaling profile to a value that is used by GCP SDK.
func ConvertToSdkAutoscalingProfile(autoscalingProfile *AutoscalingProfile) containerpb.ClusterAutoscaling_AutoscalingProfile {
	// If autoscaling profile is not specified, use the default of BALANCED.
	if autoscalingProfile == nil {
		return containerpb.ClusterAutoscaling_BALANCED
	}

	switch *autoscalingProfile {
	...
	default:
		// Unspecified is a no-op and will not change the existing autoscaling profile.
